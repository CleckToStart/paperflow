...
- Electron desktop：浅色 React 工作台 + 文件资源管理器 + 内嵌终端，用于浏览项目、启动 workflow、查看日志、浏览 artifacts、手动接管 `opencode`
...
- 点击文件后中央区只读展示文本内容
- 不提供编辑、保存、删除、移动、重命名
- 默认排除 `.git`, `node_modules`, `.venv`, `dist`, `build`, `__pycache__`, `artifacts`, `output`
- 文件读取会限制在当前 `repo_root` 内部，拒绝路径逃逸
- 单个文件预览默认限制为 1MB
- 二进制文件只显示不可预览状态
